...
function getBarHeight(count: number): number {
  const maxCount = Math.max(...Object.values(records.occupationCounts))
...
<template>
  <div>
    <h3>Occupations with income >50</h3>
    <div class="bar-chart">
      <div
        v-for="(count, occupation) in records.occupationCounts"
        :key="occupation"
        class="bar"
        :style="{ height: getBarHeight(count) + 'px' }"
      >
        <span class="bar-label">{{ occupation }}</span>
      </div>
    </div>
  </div>
</template>
